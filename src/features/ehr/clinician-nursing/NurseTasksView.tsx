...
import { completeNurseTaskAction, startNurseTaskAction } from './actions';
import type { NurseTasksData } from './tasks-data';
...
export function NurseTasksView({ data }: { data: NurseTasksData }) {
  return (
    <section className="clinician-surface">
      <header className="mb-3">
        <h2 className="h5 mb-1">Tasks</h2>
        <p className="text-muted mb-0">Handoffs, follow-ups and escalation co-signs routed to you.</p>
      </header>
            ...
            <article key={item.id} className="clinician-patient-card">
              ...
              <div className="d-flex flex-wrap gap-2">
                {canStart(item.status) ? (
                  <form action={startNurseTaskAction}>
                ...
                {canComplete(item.status) ? (
                  <form action={completeNurseTaskAction}>
                    <input type="hidden" name="taskId" value={item.id} />
