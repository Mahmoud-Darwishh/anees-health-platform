...
import type { PhysioTodayData, PhysioTodayVisit } from './data';
import {
...
function flowLabel(state: PhysioTodayVisit['flowState']): string {
...
function flowClassName(state: PhysioTodayVisit['flowState']): string {
...
function actionConfig(actionType: PhysioTodayVisit['primaryAction']) {
  if (actionType === 'acknowledge') {
    return { label: 'Acknowledge', transitionType: 'acknowledge' as const, action: acknowledgeVisitAction };
  }
  if (actionType === 'start_travel') {
    return { label: 'Start travel', transitionType: 'start_travel' as const, action: startTravelAction };
  }
  if (actionType === 'mark_arrived') {
    return { label: "I've arrived", transitionType: 'mark_arrived' as const, action: markArrivedAction };
  }
  if (actionType === 'check_in') {
    return { label: 'Check in', transitionType: 'check_in' as const, action: checkInVisitAction };
  }
  if (actionType === 'document_session') {
    return { label: 'Open session workspace', transitionType: null, action: null };
  }
  if (actionType === 'check_out') {
    return { label: 'Check out', transitionType: 'check_out' as const, action: checkOutVisitAction };
  }
  return null;
}

function timeWindow(visit: PhysioTodayVisit): string {
  return visit.scheduledTime?.trim() || 'Time not set';
}

export function TodayPageView({ data }: { data: PhysioTodayData }) {
  return (
            ...
            return (
              <article key={visit.id} className="clinician-visit-card">
                <div className="d-flex align-items-start justify-content-between gap-2 mb-2">
                  <div>
                    <p className="clinician-visit-time mb-1">{timeWindow(visit)}</p>
                ...
                {visit.transitionTimeline.length > 0 ? (
                  <div className="small text-muted mb-3">
                    {visit.transitionTimeline.slice(0, 3).map((entry) => (
                      <div key={`${visit.id}-${entry.toState}-${entry.createdAtIso}`}>
                        {workflowStateLabel(entry.toState)} · {new Date(entry.createdAtIso).toLocaleString('en-GB')}
                ...
                <div className="d-flex align-items-center gap-2 flex-wrap">
                  {config?.transitionType && visit.canTransition ? (
                    ...
                    <Link href={`/clinician/visits/${visit.id}/session`} className="btn btn-sm btn-primary">
                      {config.label}
                    </Link>
                  ) : (
