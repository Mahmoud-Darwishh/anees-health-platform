...
function actionConfig(actionType: ClinicianTodayVisit['primaryAction']) {
  if (actionType === 'acknowledge') {
    return { label: 'Acknowledge', transitionType: 'acknowledge' as const, action: doctorAcknowledgeVisitAction };
  }
  if (actionType === 'start_travel') {
    return { label: 'Start travel', transitionType: 'start_travel' as const, action: doctorStartTravelAction };
  }
  if (actionType === 'mark_arrived') {
    return { label: "I've arrived", transitionType: 'mark_arrived' as const, action: doctorMarkArrivedAction };
  }
  if (actionType === 'check_in') {
    return { label: 'Check in', transitionType: 'check_in' as const, action: doctorCheckInVisitAction };
  }
  if (actionType === 'document_session') {
    return { label: 'Open visit', transitionType: null, action: null };
  }
  // Note: getPrimaryAction never returns 'check_out' (checked_in → document_session,
  // checked_out → null). Check-out is driven from the session view, so there is no
  // check_out branch here.
  return null;
}

export function DoctorTodayView({ data }: { data: ClinicianTodayData }) {
  return (
    <section className="clinician-surface">
      ...
      {data.visits.length === 0 ? (
        <div className="alert alert-light border">No visits are assigned for today. Review + sign is on your <Link href="/clinician/doctor">My Cases</Link> queue.</div>
      ) : (
              ...
              <article key={visit.id} className="clinician-visit-card">
                ...
                <div className="d-flex align-items-center gap-2 flex-wrap">
                  {config?.transitionType && visit.canTransition ? (
                    ...
                    <Link href={`/clinician/doctor/visits/${visit.id}/session`} className="btn btn-sm btn-primary">
                      {config.label}
                    </Link>
                  ) : (
                    <Link href={`/clinician/doctor/visits/${visit.id}/session`} className="btn btn-sm btn-outline-secondary">
                      Open visit
                    </Link>
                  )}
