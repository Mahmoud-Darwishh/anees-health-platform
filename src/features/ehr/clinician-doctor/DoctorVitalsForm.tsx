...
import { recordDoctorVitalsAction } from './actions';
...
export function DoctorVitalsForm({ visitId }: { visitId: string }) {
  const [state, formAction, isPending] = useActionState(recordDoctorVitalsAction, idleDoctorFormState);
  ...
  return (
    <form action={formAction} className="clinician-doc-form">
      ...
      <div className="mb-2">
        <label htmlFor="doctor-vitals-recordedAt" className="form-label small mb-1">Recorded at</label>
        <input id="doctor-vitals-recordedAt" name="recordedAt" type="datetime-local" className="form-control form-control-sm" defaultValue={recordedAt} required />
      </div>

      <div className="row g-2">
        {NUMERIC_FIELDS.map((field) => (
          <div className="col-6 col-md-4" key={field.name}>
            <label htmlFor={`doctor-vitals-${field.name}`} className="form-label small mb-1">
